...
#roots = ['lcdm', 'g^-1 = 300','g^-1=100','g^-1=60','g^-1=30','g^-1=15']
# roots = [r'$\Lambda$CDM', r'$\Gamma_{wdm}^{-1}$ = 300 Gyr', '$\Gamma_{wdm}^{-1}$ = 100 Gyr', '$\Gamma_{wdm}^{-1}$ = 30 Gyr', '$\Gamma_{wdm}^{-1}$ = 10 Gyr']
# roots = ['Hyrec', 'Recfast']
...
ax.legend([root+': '+elem for (root, elem) in
    itertools.product(roots, y_axis)], loc='lower left', fontsize=15)

ax.set_xlabel('$\ell$', fontsize=22)
ax.tick_params(labelsize=20)

ax.set_ylabel(r'$\frac{C_\ell^{\mathrm{TT}}(\Lambda DDM)}{C_\ell^{\mathrm{TT}}(\Lambda CDM)}-1$',fontsize=25)
...
plt.savefig("epsvaried.pdf", format='pdf')
plt.show()
