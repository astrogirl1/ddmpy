...
if (pba->Omega0_dcdmdr != 0.){ /* GFA */
  // printf("pba->Omega0_dcdmdr %e\n", pba->Omega0_dcdmdr);
    ...
    pba->has_dcdm = _TRUE_;
     if (pba->Gamma_dcdm_exo != 0.) {
      fprintf(stdout, "you have chosen Gamma = %e km/s/Mpc  \n", pba->Gamma_dcdm_exo / (1.e3 / _c_));
      pba->has_dr = _TRUE_;
      }
     }
  ...
  if (pba->Omega0_dcdmdrwdm  != 0.){ /* GFA */
    ...
    pba->has_dcdm = _TRUE_;
    if (pba->Gamma_dcdm_exo != 0. && pba->m_dcdm == 0.)
      pba->has_dr = _TRUE_;
    if (pba->Gamma_dcdm_exo != 0. &&  pba->m_dcdm > 0.)
      pba->has_ncdm = _TRUE_;
  }
  ...
  /** - b) deal now with case of reading analytical function */
    ...
    */
    ...
    if(pba->background_ncdm_distribution[n_ncdm] == _fermi_dirac_){
      ...
      *f0 = pba->Gamma_dcdm_exo*n_dcdm/qcube/4/_PI_; //nb: we will divide by Hq and multiply by exp(-Gamma*tq) in background_ncdm_momenta.
  ...
  for (index_q=0; index_q<qsize; index_q++) {
    ...
    if(z> zq && background_ncdm_distribution == _massive_daughter_){
      ...
      if(background_ncdm_distribution == _decaying_neutrinos_){
        ...
        if(pba->tq_table[n_ncdm][index_q]>0 && pba->tq_table[n_ncdm][index_q]*pba->Gamma_dcdm_exo<1e-3){
          exp_factor =1;
        }
        else if(pba->tq_table[n_ncdm][index_q]>0 && pba->tq_table[n_ncdm][index_q]*pba->Gamma_dcdm_exo<10 &&  pba->tq_table[n_ncdm][index_q]*pba->Gamma_dcdm_exo>=1e-3){
          exp_factor = exp(-pba->Gamma_dcdm_exo*pba->tq_table[n_ncdm][index_q]);
        }else if(t==0 || H == 0){
          exp_factor = 0;
        }else{
          exp_factor = 0;
        ...
        // printf("exp_factor %e G %e t %e\n",exp_factor,pba->Gamma_dcdm_exo,pba->tq_table[n_ncdm][index_q]);
  ...
  */
  ...
  if (pba->has_dr == _TRUE_){
    if (pba->has_dcdm == _TRUE_){
       ...
       *
       * `f = -Omega_rad+pow(pow(Omega_rad,3./2.)+0.5*pow(a/pba->a_today,6)*pvecback_integration[pba->index_bi_rho_dcdm]*pba->Gamma_dcdm_exo/pow(pba->H0,3),2./3.);`
       *
       ...
       */
      f = 1./3.*pow(a/pba->a_today,6)*pvecback_integration[pba->index_bi_rho_dcdm]*pba->Gamma_dcdm_exo/pow(pba->H0,3)/sqrt(Omega_rad)*pba->epsilon_dcdm_wdm;
      // printf("pba->Omega0_dcdmdrwdm %e pba->Gamma_dcdm_exo %e pba->epsilon_dcdm_wdm %e \n",pba->Omega0_dcdmdrwdm,pba->Gamma_dcdm_exo,pba->epsilon_dcdm_wdm);
