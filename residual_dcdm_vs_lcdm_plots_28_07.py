...
if plot_type ==1:
    Gamma_dcdm_exo = np.array([32.679, 32.679, 32.679,32.679])
    log10_epsilon_dcdm_wdm = np.array([-2.1624, -2.0, -1.5, -1])
    log10_Gamma_dcdm_wdm = np.array([1.25, 1.25, 1.25, 1.25])
    decay_fraction = np.array([1.0, 1.0, 1.0, 1.0])

# else:
    #NEED TO MAKE CHANGES TO THE PARAMETERS HERE
    # Gamma_dcdm_exo = np.array([3.2679,9.8039, 32.679, 98.0392])
    # epsilon =  np.array([0.1,0.1,0.1, 0.1])



tau_dcdm_exo =1./(Gamma_dcdm_exo*1.02e-3)
tau_dcdm_exo
...
for i in range(4):
    ...
    # # 'decay_fraction' : 1,
    # # 'photon_energy'  : 0.01,
    # 'injected_particle_spectra' : 'dirac_photon',
    # 'injected_particle_branching_ratio' : 1,
    # 'omega_dcdmdrwdm' : 0.1194,
    # 'M_dcdm': 1,
    # 'evolver': 0,
    # 'recombination' : 'HyRec',
