...
color = ['goldenrod','orange', 'darkorange','chocolate', 'orangered', 'orchid', 'mediumorchid', 'indigo','red']
...
ylim = []
xlim = []
ax.semilogx(curve0[:, 0], abs(curve0[:, 1]), color='black', linewidth = 2.2)

for idx, dat in enumerate(data[1:]):
    index, curve = idx, dat
    # ax.semilogx(curve[:, 0], abs(curve[:, 1]), color = color[idx])
    ax.semilogx(curve[:, 0], abs(curve[:, 1]), color = color[idx], linestyle='dashed', linewidth=1.9)
...
plt.title("$\epsilon$ varied, $\Gamma_{wdm}^{-1}$ = 30 Gyr ", loc='center')
